...
\begin{enumerate}
	...
	\item Clauses with a number of literals greater than a tresshold $k$ are discarded as soon as the number of unassigned literals is greater than one \cite{grasp}.
\end{enumerate}
...
literals. For the multithreaded versions that share clauses, the two-watched literals scheme has been modified and it will be explained later in this chapter. For conflict detection and clause learning AzuDICI implements the 1-UIP algorithm. A lemma simplification algorithm is used for new learnt clauses, it's the same used in {\tt
  PicoSAT}. For search restarts it uses Luby restarts. For database database clean up it keeps binary and ternary lemmas, and more than four-literal
...
feasible when sharing n-ary clauses, because multiple threads would be modifying the position of literals. Instead, we have used a similar 
approach as used in
...
A thread clause is just a reference to an actual n-clause, which keeps extra information that the threads needs about that n-clause. It has two watched literals (WL0 and WL1), two
pointers to other thread clauses (NW0 and NW1) and a pointer to
the actual n-ary clause in the n-ary clause database that it is referencing. WL0 and WL1 keep
...
watching WL0 and WL1 respectively. The n-clause also has a set of flags, one for 
