...
In this experiment we ran the SharedNone version with our set of benchmark problems. Figure \ref{fig:xxx} shows the results for this experiment. As we expected, the cache performance decreases as we add more threads, decrease of about XX\% to XX\%. 

\subsection{AzuDICI-SharedBinaries results}

Figure \ref{fig:xxx} shows the results in this experiment and we can observe they are similar to the SharedNone results. This can be expected as we are only sharing a small amount of clauses physically and not making any significant change to the solver. The propagation scheme is still the same for both versions. Only a small improvement in cache performance can be noticed throughout the different benchmark files.

\subsection{AzuDICI-SharedAll}

The results for this last version are shown in Figure \ref{fig:xxx}. 
