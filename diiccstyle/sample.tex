...
Solvers that do not share information have the most simple design. They run completely independent solvers in parallel and wait for one of them to give an answer. Despite their simplicity, the solver \texttt{ppfolio} \cite{ppfolio}, a pure portfolio approach solver, was the winner of the crafted and random categories of the 2011 SAT competition of parallel solvers, and second place in the application category. 
...
To take our experimental claims even further, we will also measure the cache performance of the \texttt{plingeling} solver in a machine with a chip of six physical cores. As we can observe in Figure \ref{plingeling decay cache}, the miss rate of the LLC highly increases as we add more threads. This means that there will be an important performance decay caused by cache, which is what we observe in previous performance experiments.
